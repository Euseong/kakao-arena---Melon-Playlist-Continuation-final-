...
for i, songs in enumerate(val.songs):
    ...
    if not songs:
        ...
        if tags:
            ...
            song_i_pred = np.array(tag_co_occurrence_csr[:,tag_index].sum(axis=1))[:,0].argsort()[::-1]
        else:
...
for i, songs in enumerate(val.songs):
    ...
    if not songs:
        if tags:
            tag_index = list(map(lambda x : tags_dict[x], tags))
            tag_i_pred = np.array(tag_tag_co_occurrence_csr[tag_index,:].sum(axis=0))[0,:].argsort()[::-1]
